...
const { removeBackgroundFromImageBase64 } = require('remove.bg') // paid
...
const wel = JSON.parse(fs.readFileSync('./lib/welcome.json')) // paid
const nsfwgrp = JSON.parse(fs.readFileSync('./lib/nsfw.json')) // paid
...
// yes these credentials are correct and dts uses same database.
...
const downloadmp = async (sender, client, link, message, type) => {
  let uses
  let bruh
  ...
  try {
    await connection.awaitQuery(`SELECT uses FROM bruhmomento WHERE user = '${sender.id}'`, (err, result) => {
      console.log('finding uses')
      ...
      if (err) {
        ...
        try {
          ...
          console.log('inserting uses')
          var sql = `INSERT INTO bruhmomento (user, uses) VALUES ('${sender.id}', '1')`
          connection.awaitQuery(sql, (err, result) => {
            if (err) {
              console.log(err)
            }
      ...
      console.log(`after func -> ${uses}`)

      if (parseInt(uses) >= limit) {
        client.reply(message.from, 'Limit reached', message.id)
        return
      }

      client.reply(message.from, 'Downloading...', message.id)

      if (type === 'mp4') {
        const filename = `${Math.random().toString(36).substring(7)}.mp4`
        const stream = fs2.createWriteStream(`./media/${filename}`)

        stream.on('finish', () => {
          console.log('filename -> ' + `./media/${filename}`)
          const base64 = fs.readFileSync(`./media/${filename}`, {
            encoding: 'base64'
          })
          client.sendFile(message.from, `data:video/mp4;base64,${base64}`, 'filename.mp4', 'Lord Zeref')
        })
        ...
        const filename2 = `${Math.random().toString(36).substring(7)}.mp3`
        const stream2 = fs2.createWriteStream(`./media/${filename2}`)

        stream2.on('finish', () => {
          console.log('filename -> ' + `./media/${filename2}`)
          const base642 = fs.readFileSync(`./media/${filename2}`, {
            encoding: 'base64'
          })
          client.sendFile(message.from, `data:audio/mp3;base64,${base642}`, `${filename2}`, 'Lord Zeref')
        })
      ...
      if (bruh !== 'error') { updateuse(sender.id, uses) }
    })
  } catch (e) {
    console.log(e)
  }
...
const updateuse = (from, uses) =>  {
  const intuse = parseInt(uses) + 1
  uses = intuse.toString()
  if (uses == 'NaN') { uses = '1' }
  console.log('inserting uses')
  var sql = `UPDATE bruhmomento SET uses = '${uses}' WHERE user = '${from}'`
  connection.awaitQuery(sql, (err, result) => {
    if (err) {
      console.log(err)
    }
...
const delall = () =>  {
  ...
  var sql = 'TRUNCATE TABLE bruhmomento'
  connection.awaitQuery(sql, (err, result) => {
    if (err) {
      console.log(err)
    }
...
module.exports = msgHandler = async (client, message) => {
  try {
    ...
    if (isCmd && isGroupMsg) console.log(chat.groupMetadata.id), console.log(color('[EXEC]'), color(time, 'yellow'), color(`${command} [${args.length}]`), 'from', color(pushname), 'in', color(name))
    const botNumber = await client.getHostNumber()
    const groupId = isGroupMsg ? chat.groupMetadata.id : ''
    const groupAdmins = isGroupMsg ? await client.getGroupAdmins(groupId) : ''
    const isGroupAdmins = isGroupMsg ? groupAdmins.includes(sender.id) : false
    const isBotGroupAdmins = isGroupMsg ? groupAdmins.includes(botNumber + '@c.us') : false
    const botadmins = await client.getGroupAdmins('27656035811-1600249425@g.us')
    const isbotadmin = botadmins.includes(sender.id)
    const isBanned = ban.includes(chatId)
    const paidusers = ['966572592040@c.us', '27656035811@c.us', '2348100626435@c.us', '919051110368@c.us', '6283151228661@c.us', '358456614448@c.us']
    const ispaid = paidusers.includes(sender.id)
    const owner = '966572592040' // eg 966xxxxxxxx
    ...
    if (!isBanned) {
      switch (command) {
        ...
        case 'tsticker':
          ...
          if (isMedia && type == 'image') {
            try {
              ...
              const result = await removeBackgroundFromImageBase64({ base64img, apiKey: '1h5qrmV54HeDweamVoWs2ac2', size: 'auto', type: 'auto', outFile })
          ...
          break
        case 'grouplink':
          if (!isGroupAdmins) return await client.reply(from, 'Only group admins can use this commad', id)
          if (isGroupMsg) {
            const inviteLink = await client.getGroupInviteLink(groupId)
            client.sendLinkWithAutoPreview(from, inviteLink, `\n🔖Group link :  *${name}*`)
          } else {
            	await client.reply(from, 'This command can only be used in groups!', id)
          }
          break
        case 'ownergroup':
        ...
        case 'set_limit':
          var arg3 = body.trim().split(' ')
          limit = parseInt(arg3[1])
          console.log(limit)
          break

        case 'mp4':
          console.log('Got mp4')
          var arg = body.trim().split(' ')

          console.log(sender.id)
          if (arg.length >= 1 && arg[1] !== undefined) {
            if (arg[1].includes('youtube.com') || arg[1].includes('youtu.be')) {
              await downloadmp(sender, client, arg[1], message, 'mp4')
            }
          } else {
            client.reply(from, 'No link gotten', message.id)
          }
          break
        ...
        case 'mp3':
          ...
          if (arg.length >= 1 && arg[1] !== undefined) {
            if (arg[1].includes('youtube.com') || arg[1].includes('youtu.be')) {
              await downloadmp(sender, client, arg[1], message, 'shelltear')
            }
          } else {
            client.reply(from, 'No link gotten', message.id)
          }
        ...
        case 'delall':
          client.reply(from, 'Deleting entrys...', message.id)
          delall()
          break
...
${desc}`)
        ...
        case 'bc':
          ...
          const msg = body.slice(4)
          const chatz = await client.getAllChatIds()
          for (const ids of chatz) {
            var cvk = await client.getChatById(ids)
            if (!cvk.isReadOnly) client.sendText(ids, `[ DTS BOT Broadcast ]\n\n${msg}`)
          }
        ...
        case 'ban_user':
          ...
          for (let i = 0; i < mentionedJidList.length; i++) {
            ...
            client.reply(from, '*User banned!✨*!', message.id)
          }
          break
        case 'covid':
          arg = body.trim().split(' ')
        ...
        // paid
        case 'unregister':
          if (!ispaid) return client.reply(from, 'Only paid users can use this command', id)
          arg = body.trim().split(' ')
          if (!isGroupAdmins) return client.reply(from, 'Only Admins can use this command', id)
          if (arg[1] == 'welcome') {
            const inx = ban.indexOf(from)
            wel.splice(inx, 1)
            fs.writeFileSync('./lib/welcome.json', JSON.stringify(wel))
        ...
        case 'sr':
          ...
          try {
            ...
            if (nsfw === true) {
              if ((ispaid) && (isnsfw)) {
                ...
                await client.reply(from, `Hentai is not registered on *${name}*`, id)
              }
            } else {
              await client.sendFileFromUrl(from, `${url}`, 'Reddit.jpg', `${title}` + '\n\nPostlink:' + `${postLink}`)
            }
          ...
          break
        case 'ban_false':
          if (!isowner) return client.reply(from, 'Only bot admins can use this command', message.id)
          const inx = ban.indexOf(mentionedJidList[0])
          ban.splice(inx, 1)
          fs.writeFileSync('./lib/banned.json', JSON.stringify(ban))
          client.reply(from, '*User unbanned!✨!*', message.id)
          break
        case 'remove':
          if (!isGroupMsg) return client.reply(from, '...', message.id)
          ...
          for (let i = 0; i < mentionedJidList.length; i++) {
            if (groupAdmins.includes(mentionedJidList[i])) return await client.reply(from, 'Failed To remove: *_USER_* is admin of this group!', message.id)
        ...
        case 'leave':
          ...
          await client.sendText(from, 'Thanks for using *lօɾժ zҽɾҽբ* Sayonara!').then(() => client.leaveGroup(groupId))
          break
        case 'promote':
          if (!isGroupMsg) return client.reply(from, '.', message.id)
          if (!isGroupAdmins) return client.reply(from, 'You are not an admin', message.id)
...
🌐️ *URL*: ${url}`
        ...
        case 'inu':
          ...
          const kya = list[Math.floor(Math.random() * list.length)]
          client.sendFileFromUrl(from, kya, 'Dog.jpeg', 'Doggo ✨️', id)
          break
        case 'neko':
          var q2 = Math.floor(Math.random() * 900) + 300
        ...
        case 'rpaper' :
          ...
          const walnimek = walnime[Math.floor(Math.random() * walnime.length)]
        ...
        case 'profile':
          let lordzeref
          await connection.awaitQuery(`SELECT uses FROM bruhmomento WHERE user = '${sender.id}'`, async (err, result) => {
            console.log('finding uses, profile command')
            if (err) {
              ...
              try {
                ...
                console.log('error getting uses', e)
                lordzeref = 'error'
              }
            }
            var role = 'User'
            ...
            if (isGroupMsg) {
              if (!quotedMsg) {
                ...
                await client.sendFileFromUrl(from, pfp, 'pfp.jpg', `*User Profile* ✨️ \n\n*❢ Name ❢:* ${namae}\n\n*➽ Ping limit ➽:* 2 / 0\n\n*Youtube limit:* ${lordzeref}/${limit}\n\n◔ User Type ◔:* None\n\n*♕Role:* ${role}\n\n*⎙ User Notice ⎙:* ${status}\n\n*⊝ BLOCKED ⊝:* ${block}\n\n*♛ Admin ♛:* ${adm}`)
              } else if (quotedMsg) {
                var qmid = quotedMsgObj.sender.id
                block = ban.includes(qmid)
                pic = await client.getProfilePicFromServer(qmid)
                namae = quotedMsgObj.sender.name
                sts = await client.getStatus(qmid)
                adm = isGroupAdmins
                const { status } = sts
                if (pic === undefined) {
                  pfp = errorurl
